...
**One file with everything** — tables, RPCs, RLS, payments, v1.4 features:

1. Open [Supabase SQL Editor](https://supabase.com/dashboard/project/siwzjxwholmoassrdtwx/sql/new)
2. Open **`TrotroOS_COMPLETE.sql`** in this folder (≈2,250 lines)
3. **Ctrl+A → Ctrl+C → paste → Run**
4. **Settings → API → Reload schema**, then reload the app (`r` in Metro)

Regenerate after SQL changes: `npm run build:complete-sql`

---

## One-time setup (legacy / partial files)
...
**Baseline RLS only (your policy list, TrotroOS-adapted)?** Run **`FIX_baseline_rls_policies.sql`**. Helper: `.\scripts\fix-baseline-rls.ps1`. Do **not** paste generic `auth.uid()` policies — passengers use device IDs.

**`cannot alter type of a column used in a policy definition` on reservations?** Run **`FIX_reservations_passenger_id_alter.sql`** (drops policies → alters `passenger_id` → recreates policies). Or the full **`FIX_passenger_profiles_and_reservations.sql`**.
